...
func void Use_Bookstand_Cave_Sekret_01_S1()
{
	...
	if((Hlp_GetInstanceID(self) == Hlp_GetInstanceID(her)) || (self.id == 0) || Npc_IsPlayer(self))
	{
		...
		else if(PLAYER_TALENT_FOREIGNLANGUAGE[ORC_LANGUAGE_1] == true)
		{
			...
			Doc_PrintLine(nDocID,0,"");
			Doc_PrintLine(nDocID,0,"");
			Doc_PrintLines(nDocID,0,"Здесь нарисован человек, который молится перед алтарем.");
			Doc_PrintLine(nDocID,0,"");
			Doc_PrintLines(nDocID,0,"");
			Doc_PrintLine(nDocID,0,"");
			Doc_Show(StPl_nDocID);
...
func void Use_Bookstand_Cave_Sekret_02_S1()
{
	...
	if((Hlp_GetInstanceID(self) == Hlp_GetInstanceID(her)) || (self.id == 0) || Npc_IsPlayer(self))
	{
		...
		else if(PLAYER_TALENT_FOREIGNLANGUAGE[ORC_LANGUAGE_1] == true)
		{
			...
			Doc_PrintLine(nDocID,0,"");
			Doc_PrintLine(nDocID,0,"");
			Doc_PrintLines(nDocID,0,"Текст очень древний, трудно что-то разобрать, но последнюю строчку очень четко видно: ”Айд терфт ймнырт”. Что это может значить?");
			Doc_PrintLine(nDocID,0,"");
			Doc_PrintLines(nDocID,0,"");
			Doc_PrintLine(nDocID,0,"");
			Doc_Show(nDocID);
