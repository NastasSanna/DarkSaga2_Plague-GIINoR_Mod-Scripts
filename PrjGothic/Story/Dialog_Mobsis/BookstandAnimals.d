...
func void Use_BookstandAnimals1_S1()
{
	...
	if((Hlp_GetInstanceID(self) == Hlp_GetInstanceID(her)) || (self.id == 0) || Npc_IsPlayer(self))
	{
		...
		Doc_PrintLines(nDocID,1,"Очень опытные охотники могут также снимать шкуры с болотных акул и шныгов.");
		Doc_PrintLine(nDocID,1,"");
		Doc_PrintLine(nDocID,1,"");
		Doc_PrintLine(nDocID,1,"Изъятие когтей");
		Doc_PrintLines(nDocID,1,"Это искусство применимо ко всем типам ящеров, снепперам, шныгам и мракорисам.");
		Doc_Show(nDocID);
...
func void Use_BookstandAnimals2_S1()
{
	...
	if((Hlp_GetInstanceID(self) == Hlp_GetInstanceID(her)) || (self.id == 0) || Npc_IsPlayer(self))
	{
		...
		Doc_PrintLine(nDocID,1,"");
		Doc_PrintLine(nDocID,1,"");
		Doc_PrintLines(nDocID,1,"Полевые хищники и ползуны атакуют при помощи жвал. Жвала ползунов");
		...
		Doc_PrintLines(nDocID,1,"Панцири ползунов также весьма популярны. Из них можно изготавливать доспехи.");
		Doc_PrintLine(nDocID,1,"");
