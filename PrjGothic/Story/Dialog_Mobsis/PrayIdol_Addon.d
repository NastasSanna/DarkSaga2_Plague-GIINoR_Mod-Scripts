...
func void b_DS_GiveLP_InsteadOfHP(var C_NPC her, var int lp, var int hp_max)
{
	if(her.attribute[ATR_HITPOINTS_MAX] >= (hp_max + 2) )
	{
		her.lp += lp;
		her.attribute[ATR_HITPOINTS_MAX] -= hp_max;
		if(her.attribute[ATR_HITPOINTS] >= hp_max + 2)
		{
			her.attribute[ATR_HITPOINTS] -= hp_max;
		}
		else
		{
			her.attribute[ATR_HITPOINTS] = 2;
		};
	}
	else
	{
		Print("У вас мало жизненной энергии!");
	};
};
instance PC_PrayIdol_GiveLP(C_Info)
{
	npc = PC_Hero;
	nr = 20;
	condition = PC_PrayIdol_GiveLP_Condition;
	information = PC_PrayIdol_GiveLP_Info;
	permanent = true;
	description = "Белиар, поделись своей мудростью.";
};
func int PC_PrayIdol_GiveLP_Condition()
{
	if(PLAYER_MOBSI_PRODUCTION == MOBSI_PrayIdol)
	{
		return TRUE;
...
func void PC_PrayIdol_GiveLP_Info()
{
	Info_ClearChoices(PC_PrayIdol_GiveLP);
	Info_AddChoice(PC_PrayIdol_GiveLP,Dialog_Back,PC_PrayIdol_GiveLP_Back);
	Info_AddChoice(PC_PrayIdol_GiveLP,"Отдать 39 очков жизни (получить 3 очка опыта)",PC_PrayIdol_GiveLP_3LP);
	Info_AddChoice(PC_PrayIdol_GiveLP,"Отдать 28 очков жизни (получить 2 очка опыта)",PC_PrayIdol_GiveLP_2LP);
	Info_AddChoice(PC_PrayIdol_GiveLP,"Отдать 15 очков жизни (получить 1 очко опыта)",PC_PrayIdol_GiveLP_1LP);	
};
func void PC_PrayIdol_GiveLP_Back()
{
	Info_ClearChoices(PC_PrayIdol_GiveLP);
};
func void PC_PrayIdol_GiveLP_1LP()
{
	b_DS_GiveLP_InsteadOfHP(self,1,10);
	PC_PrayIdol_GiveLP_Info();
};
func void PC_PrayIdol_GiveLP_2LP()
{
	b_DS_GiveLP_InsteadOfHP(self,2,20);
	PC_PrayIdol_GiveLP_Info();
};
func void PC_PrayIdol_GiveLP_3LP()
{
	b_DS_GiveLP_InsteadOfHP(self,3,30);
	PC_PrayIdol_GiveLP_Info();
};
