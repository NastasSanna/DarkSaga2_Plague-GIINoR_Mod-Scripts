...
func string C_DS_GetRandomPointName(var string PointTemplate, var int insPointNum)
{
	var string insPoint;
	if (insPointNum < 10)
	{
...
func int B_DS_WldSpawnItem(var int ItemInst, var string PointTemplate, var int MinNum, var int MaxNum)
{
...
// есть вариант с LeGo и без него (через рекурсию), оба работают
...
func int B_DS_WldSpawnItems(var int ItemInst, var string PointTemplate, var int MinNum, var int MaxNum, var int Count)
{
	...
	{
		Wld_InsertItem(ItemInst, C_DS_GetRandomPointName(PointTemplate, MinNum));
		return 1 + B_DS_WldSpawnItems(ItemInst, PointTemplate, MinNum + 1, MaxNum, Count - 1);	
	};
