...
{
	AI_Output(other,self,"DIA_Ekor_TEACH_15_00");	//Каким рецептам ты можешь обучить меня?
	...
	{
		AI_Output(self,other,"DIA_Ekor_TEACH_08_00");	//Я больше ничему не могу научить тебя.
	}
	else
	{
		AI_Output(self,other,"DIA_Ekor_TEACH_08_01");	//Есть несколько - выбирай.
...
func void dia_Ekor_teach_health01()
{
	...
	if(c_Npc_DS_HasItems_Scr(other,B_DS_GetTeachPaymentType(),x,YPos_5,TRUE))
	{
		...
		{
			AI_Output(self,other,"DIA_Ekor_TEACH_Health01_08_00");	//Ингредиенты для лечебной эссенции - лечебные травы и луговой горец.
	...
	{
		AI_Output(self,other,"DIA_Ekor_NoGold_08_00");	//Сначала золото.
	};
	Info_ClearChoices(dia_Ekor_teach);
};
func void dia_Ekor_teach_health02()
{
	...
	if(c_Npc_DS_HasItems_Scr(other,B_DS_GetTeachPaymentType(),x,YPos_5,TRUE))
	{
		...
		{
			AI_Output(self,other,"DIA_Ekor_TEACH_Health02_08_00");	//Чтобы приготовить лечебный экстракт, тебе понадобятся лечебные растения и луговой горец.
			AI_Output(self,other,"DIA_Ekor_TEACH_Health02_08_01");	//Кипятить этот экстракт нужно очень осторожно.
			b_giveInvItems(other,self,B_DS_GetTeachPaymentType(),x);
			B_DS_ClearGold();
		};
	}
...
func void dia_Ekor_teach_health03()
{
	...
	if(c_Npc_DS_HasItems_Scr(other,B_DS_GetTeachPaymentType(),x,YPos_5,TRUE))
	{
		...
		{
			AI_Output(self,other,"DIA_Ekor_TEACH_Health03_08_00");	//Для создания лечебного эликсира требуется чуть больше опыта.
			AI_Output(self,other,"DIA_Ekor_TEACH_Health03_08_01");	//Тебе понадобятся лечебные корни и луговой горец. При кипячении этого зелья нужно быть особенно внимательным.
			b_giveInvItems(other,self,B_DS_GetTeachPaymentType(),x);
			B_DS_ClearGold();
		};
	}
...
func void dia_Ekor_teach_mana01()
{
	...
	if(c_Npc_DS_HasItems_Scr(other,B_DS_GetTeachPaymentType(),x,YPos_5,TRUE))
	{
		...
		{
			AI_Output(self,other,"DIA_Ekor_TEACH_Mana01_08_00");	//Эссенция маны - самое простое из магических зелий.
			AI_Output(self,other,"DIA_Ekor_TEACH_Mana01_08_01");	//Возьми огненную крапиву и луговой горец и вскипяти их на медленном огне.
			b_giveInvItems(other,self,B_DS_GetTeachPaymentType(),x);
			B_DS_ClearGold();
		};
	}
...
func void dia_Ekor_teach_mana02()
{
	...
	if(c_Npc_DS_HasItems_Scr(other,B_DS_GetTeachPaymentType(),x,YPos_5,TRUE))
	{
		...
		{
			AI_Output(self,other,"DIA_Ekor_TEACH_Mana02_08_00");	//Так как ты уже умеешь готовить эссенцию маны, если немного потренируешься, то сможешь готовить также и экстракт.
			AI_Output(self,other,"DIA_Ekor_TEACH_Mana02_08_01");	//Единственно, нужно уделить особое внимание процессу выпаривания этого зелья. В качестве ингредиентов для него используются огненная трава и луговой горец.
			b_giveInvItems(other,self,B_DS_GetTeachPaymentType(),x);
			B_DS_ClearGold();
		};
	}
...
func void dia_Ekor_teach_mana03()
{
	...
	if(c_Npc_DS_HasItems_Scr(other,B_DS_GetTeachPaymentType(),x,YPos_5,TRUE))
	{
		...
		{
			AI_Output(self,other,"DIA_Ekor_TEACH_Mana03_08_00");	//Так как ты уже уметь готовить экстакт маны, то смочь готовить эликсир.
			AI_Output(self,other,"DIA_Ekor_TEACH_Mana03_08_01");	//Но следует быть особенно внимательным при кипячении этого зелья. В качестве ингредиентов для него используются огненные корни и луговой горец.
			b_giveInvItems(other,self,B_DS_GetTeachPaymentType(),x);
			B_DS_ClearGold();
		};
	}
...
func void dia_Ekor_teach_permstr()
{
	...
	{	
		...
		{
			AI_Output(self,other,"DIA_Ekor_TEACH_PermSTR_08_00");	//Эликсир силы! Превосходное зелье. Для него тебе понадобятся редкий драконий корень и царский щавель.
			AI_Output(self,other,"DIA_Ekor_TEACH_PermSTR_08_01");	//При кипячении этого зелья следи, чтобы пузырьки не стали слишком большими, иначе тебя будет ждать неприятный сюрприз!
			b_giveInvItems(other,self,B_DS_GetTeachPaymentType(),x);
			B_DS_ClearGold();
		};
	}
