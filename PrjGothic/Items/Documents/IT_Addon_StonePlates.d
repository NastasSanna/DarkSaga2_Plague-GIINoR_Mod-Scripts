...
instance ITWR_ANCIENTSTONEPLATE(C_Item)
{
	...
	on_state[0] = use_ancientstoneplate;
	scemeName = "MAP";
	inv_rotx = -90;
	inv_roty = 0;
	inv_rotz = 0;
	description = name;
	text[2] = "Каменная табличка";
	text[5] = NAME_Value;
	count[5] = value_StonePlateCommon;
};
func void use_ancientstoneplate()
{
	var int nDocID;
	nDocID = Doc_Create();
...
instance ITWR_ANCIENTSTONEPLATE_PERMSTRDEX(C_Item)
{
	...
	visual = "ItMi_StonePlate_Read_06.3ds";
	material = MAT_LEATHER;
	on_state[0] = use_ancientstoneplate_permstrdex;
	scemeName = "MAPSEALED";
	inv_rotx = -90;
	inv_roty = 0;
	inv_rotz = 0;
...
func void use_ancientstoneplate_permstrdex()
{
	self.attribute[ATR_STRENGTH] += 2;
	self.attribute[ATR_DEXTERITY] += 2;
	Wld_PlayEffect("spellFX_LIGHTSTAR_BLUE",hero,hero,0,0,0,FALSE);
	Snd_Play("SFX_HealObsession");
	PrintScreen(PRINT_LEARNSTR2,-1,48,FONT_Screen,2);
	PrintScreen(PRINT_LEARNDEX2,-1,52,FONT_Screen,2);
};
