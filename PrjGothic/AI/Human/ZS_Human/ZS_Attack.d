...
func int ZS_Attack_Loop()
{
	...
	if(Npc_GetDistToNpc(self,other) > self.aivar[AIV_FightDistCancel])
	{
		...
		//NS - 18/07/2013
		B_DS2P_CheckLog_OnFightCanceled(self, other);	//сюда пишем все квестовые проверки
		return LOOP_END;
	};
	...
	if(self.aivar[AIV_WaitBeforeAttack] >= 1)
	{
		var int x;
		var float y;
		x = self.aivar[AIV_WaitBeforeAttack];
		y = IntToFloat(x);
		//AI_Wait(self,0.8);
		AI_Wait(self,y);
	...
	else
	{
		...
		if(Hlp_IsValidNpc(other) && !C_NpcIsDown(other) && ((Npc_GetDistToNpc(self,other) < PERC_DIST_INTERMEDIAT) || Npc_IsPlayer(other)) 
		&& (Npc_GetHeightToNpc(self,other) < PERC_DIST_HEIGHT) && (other.aivar[AIV_INVINCIBLE] == FALSE) 
...
func void ZS_Attack_End()
{
	...
	&& (Npc_GetDistToNpc(self,other) < PERC_DIST_INTERMEDIAT))
	{
		...
		else if(C_NpcIs(self, BAU_105_DS2P_Tamir) && (Npc_HasItems(other,ItFoMuttonRaw) > 0))
		{
			AI_StartState(self,ZS_GetMeat,0,"");
			return;
		}
		else if(C_NpcIs(self,PIR_202_DS2P_Symon) && (other.guild == GIL_SHEEP))
		{
			AI_StartState(self,ZS_GetMeat,0,"");
			return;
		};
	...
	{
		CreateInvItems(self,ItFo_DS_WaterMega,2);
