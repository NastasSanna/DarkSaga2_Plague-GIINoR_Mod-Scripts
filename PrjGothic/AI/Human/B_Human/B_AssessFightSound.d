...
func void B_AssessFightSound()
{
	...
	//*NS - 09/07/2013 они не живые
	If (victim.guild == GIL_PUBLIC)
	{
		return;
	};
	//*/
	...
	{
		if(C_BodyStateContains(other,BS_SWIM) || C_BodyStateContains(other,BS_DIVE) || C_BodyStateContains(victim,BS_SWIM) 
	...
	if((victim.aivar[AIV_SECURITY] == TRUE) && Npc_IsPlayer(other) && (self.aivar[AIV_SECURITY] == TRUE))
	{
		B_Attack(self,other,victim.aivar[AIV_ATTACKREASON],1);
		return;
	};
	...
	if((victim.guild > GIL_SEPERATOR_HUM) && (victim.aivar[AIV_MM_REAL_ID] != ID_MEATBUG) && 
	(Npc_GetAttitude(self,other) != ATT_HOSTILE) && !Npc_IsDead(victim))
	{
	...
	if((C_PlayerIsFakeBandit(self,other) || C_PlayerIsFakeBandit(self,victim)) && ((other.guild == GIL_BDT) 
	|| (victim.guild == GIL_BDT)))
	{
		if(self.guild == GIL_BDT)
		{
			...
			/*NS - 09/03/16 ¬џ–≈«јЌќ, AIV_StoryBandit заменена на AIV_StoryFlags
	...
	if(((other.aivar[AIV_ATTACKREASON] == AR_GuardStopsIntruder) || (other.aivar[AIV_ATTACKREASON] == AR_MonsterCloseToGate) || (other.aivar[AIV_ATTACKREASON] == AR_HumanMurderedHuman) 
	...
	if(((victim.aivar[AIV_ATTACKREASON] == AR_GuardStopsIntruder) || (victim.aivar[AIV_ATTACKREASON] == AR_MonsterCloseToGate) 
	|| (victim.aivar[AIV_ATTACKREASON] == AR_HumanMurderedHuman) 
	|| (victim.aivar[AIV_ATTACKREASON] == AR_GuildEnemy) || (victim.aivar[AIV_ATTACKREASON] == AR_GuardCalledToKill)) && (Npc_GetAttitude(self,victim) == ATT_FRIENDLY))
	{
	...
	if(((other.aivar[AIV_ATTACKREASON] == AR_UseMob) || (other.aivar[AIV_ATTACKREASON] == AR_Theft) 
	|| (other.aivar[AIV_ATTACKREASON] == AR_LeftPortalRoom)) && (Npc_GetAttitude(self,other) == ATT_FRIENDLY))
	{
	...
	if(((victim.aivar[AIV_ATTACKREASON] == AR_UseMob) || (victim.aivar[AIV_ATTACKREASON] == AR_Theft) 
	|| (victim.aivar[AIV_ATTACKREASON] == AR_LeftPortalRoom)) && (Npc_GetAttitude(self,victim) == ATT_FRIENDLY))
	{
