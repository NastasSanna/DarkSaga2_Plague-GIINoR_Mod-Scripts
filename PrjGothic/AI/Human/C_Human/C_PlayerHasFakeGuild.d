...
func int C_PlayerHasFakeGuild(var C_Npc slf,var C_Npc oth)
{
	...
	else if(slf.guild == oth.guild)
	{
		...
		if((oth.guild == GIL_NONE) && ((Hlp_IsItem(itm,ITAR_Vlk_L) == TRUE) || (Hlp_IsItem(itm,ITAR_Vlk_M) == TRUE) || (Hlp_IsItem(itm,ITAR_Vlk_H) == TRUE) || (Hlp_IsItem(itm,ITAR_Bau_L) == TRUE) || (Hlp_IsItem(itm,ITAR_Bau_M) == TRUE) || (Hlp_IsItem(itm,ITAR_Leather_L) == TRUE) || (Hlp_IsItem(itm,ITAR_Prisoner) == TRUE) || (Hlp_IsItem(itm,itar_ds_prisoner) == TRUE)))
		{
			return FALSE;
		}
