...
const int AR_SQUEEZY = 20;			//Redleha:вымогательство
...
//REDLEHA/10.12.2013
//Если приспичит, можно переделать во флаги. AIV_MM_*[START|END] START=0;END=1;
...
//Redleha:новые животные----
const int ID_NEW_MONSTER = 65; //пустой слот
const int ID_GOBBO_MAGE = 66;	//гоблин-шаман
...
const int ID_SLEEPKEEPER = 72;	//Ловчий
const int ID_GNORE = 73;		//рудогрыз
...
const int ID_WISP = 80;	//NS - и почему его не было?
//--------------------------
...
//ЭТИ ДВА АИВАРА можно использовать
const int AIV_StoryFlags = 74; //NS - 08/03/16 различные квестовые флаги, можно сразу несколько. проверять через HasFlag
	const int AIV_StoryFlag_SwampGoblin = 1; //гоблин с болота
...
//Redleha: Добавленные мной aivar-ы
...
//const int AIV_Name = 95; --> в файле UserConst !НЕ БРАТЬ
const int AIV_TALK_TRIA = 99;	//NS: триалог без LeGo
...
//Типы НПС - определяется принадлежность
...
//163-175 - лица феррокийцев с татушками и без.
