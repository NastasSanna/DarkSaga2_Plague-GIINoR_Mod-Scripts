...
var int ARR_oCInfos_ADR;		//массив диалогов (объектов класса oCInfo)
...
func void oCInfo_AddChoice(var C_INFO info,var string addTopic,var int nr/*var string funcName*/)	//NOT WORK
{
	...
	str = addTopic;
	zStr = _^(_@s(str));
	//nr = MEM_FindParserSymbol("dia_BlaBlaBla");
	CALL_IntParam(nr);
	CALL_StructParam(MEM_InstToPtr(zStr), sizeof(zStr));
	var int x; x = MEM_InstToPtr(info)-oCInfo_C_INFO_Offset;
	CALL__thiscall(x,7355168);
};
...
///**************INVENTORY_COLS_ROWS**************************///
//
const int invMaxCols_Adr = 11211028;	//00AB1114	//short(2 bytes)
const int invMaxRows_Adr = 11211030;	//00AB1116	//short(2 bytes)
var int InvMaxColsRows[2];
const int Cols_Index=0;
const int Rows_Index=1;

func int MEM_ReadShort(var int adr) {
   return MEM_ReadInt(adr) & 65535;
};
func void MEM_WriteShort (var int adr, var int val) {
   if (val & ~ 65535) {
      MEM_Warn ("MEM_WriteShort: Val out of range! Truncating to 16 bits.");
      val = val & 65535;
   };   
   MEM_WriteInt (adr, (MEM_ReadInt (adr) & ~ 65535) | val);
};

func int Inv_GetCols(){
	return MEM_ReadShort(invMaxCols_Adr);
};
func void Inv_ChangeCols(var int newColsSize){
	MEM_WriteShort(invMaxCols_Adr,newColsSize);
};
func int Inv_GetRows(){
	return MEM_ReadShort(invMaxRows_Adr);
};
func void Inv_ChangeRows(var int newRowsSize){
	MEM_WriteShort(invMaxRows_Adr,newRowsSize);
};

func void Inv_GetColsRows()
{
	InvMaxColsRows[Cols_Index] = Inv_GetCols();
	InvMaxColsRows[Rows_Index] = Inv_GetRows();
};
func void Inv_ChangeColsRows(var int newColsSize,var int newRowsSize){
	MEM_WriteShort(invMaxCols_Adr,newColsSize);
	MEM_WriteShort(invMaxRows_Adr,newRowsSize);
};
