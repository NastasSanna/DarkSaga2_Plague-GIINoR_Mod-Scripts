...
//##    ================                                                                ##
//##                                                                                    ##
//##    Dieses Script ermoglicht die Erstellung von mehrseitigen Buchern, die           ##
//##    per Tastendruck (Pfeiltasten links und rechts) durchgeblattert werden konnen.   ##
//##    In jedem "Multibuch" muss in der On_State Funktion "B_MultiPageBooks"           ##
//##    aufgerufen werden. Als Parameter erwartet die Funktion einen String,            ##
//##    der den Funktionsnamen der Buchseiten enthalt (GRO? GESCHRIEBEN!). In der       ##
//##    neuen Version wird keine Seitenzahl mehr benotigt, sie wird nun vom Spiel       ##
//##    selbst erkannt. Au?erdem konnen nun alle Seiten in eine einzige Funktion        ##
//##    geschrieben werden und das ganze System ist nun denke ich verstandlicher        ##
//##                                                                                    ##
//########################################################################################
...
// Flags (konnen uber mDoc_Create(var int flag) gesetzt werden
const int DOC_FLAG_NONE = 0; // Nichts
const int DOC_FLAG_LOOP = 1; // Buch fangt hinten wieder von vorne an
...
// Wird fur Tastatureingaben verwendet

func void ZS_MultiPageBooks() {
    //Damit sich der hero nicht beim umblattern dreht^^
    var oCNpc her;
    her = Hlp_GetNpc(hero);
...
func int ZS_MultiPageBooks_Loop() {
    ...
    // Wenn Escape gedruckt, Loop verlassen und Buch schlie?en
...
func void ZS_MultiPageBooks_End()
{
    ...
    // Und den Buchhelfer wieder ins jenseits befordern
    B_RemoveNpc (self);
};

//----------------------------------
//    Die eigentliche Funktion
//    um ein Buch zu offnen
//----------------------------------
...
// Die Namen wurden alle beibehalten, es wurde nur uberall ein 'm' vorne angehangt (m wie multi)
...
// Neu: Seitenzahl wird nicht mehr benotigt
func void mDoc_SetPages(var int id) {
    Doc_SetPages(id, 2);
};

// Neu: Mehr als nur 0, 1 bei 'page' moglich
...
// Neu: Mehr als nur -1, 0 und 1 bei 'page' moglich
...
// Neue Hilfsfunktion: Erstellt ein normales Buch (es fehlen nur noch Hintergrunde)
...
// Unverandert [existiert nur der Ordnung halber]
func void mDoc_Show(var int id) {
    // Uberflussig ich wei? :p
    Doc_Show(id);
};
...
//    MULTITESTBUCH_01
//-------------------------------
// Hier wird ein Buch so einfach und so kurz wie Moglich geschrieben
instance _01_MultiTestBuch(C_Item) 
{
    ...
    scemeName    =    "MAP";    
    description  =    "Zum Testen von mehrseitigen Buchern";
...
func void MULTITESTBUCH_01_CONTENT() {
    ...
    // verwendet, dadurch werden die hier angegebenen Hintergrunde
    // auf jeder Seite angezeigt und sie mussen nur einmalig hier
    ...
    // konnen nun aber auch mehr als 0 und 1 erfolgen:
...
//    MULTITESTBUCH_02
//-------------------------------
// Hier wird ein Buch mit (fast ;-) ) allen Moglichkeiten die dieses Pack
// bietet angezeigt:
...
func void MULTITESTBUCH_02_CONTENT() {
    ...
    // Diesmal kommen Hintergrunde, Fonts und Margins zu jeder
    ...
    mDoc_PrintLine  (DocID, 1, "");
    mDoc_PrintLine  (DocID, 1, "");
    mDoc_PrintLines (DocID, 1, "Unterschiedliche Hintergrunde,");
    ...
    mDoc_PrintLine  (DocID, 4, "");
    mDoc_PrintLines (DocID, 4, "Und jetzt der Knuller: Ich wei? in welche Richtung du Blatterst ;)");
    ...
    mDoc_PrintLine  (DocID, 9, "");
    mDoc_PrintLine  (DocID, 9, "");
    mDoc_PrintLines (DocID, 9, "Ja und das wars auch schon :) Viel Spa?! [Dank dem Loop kommst du jetzt wieder auf Seite 1]");
