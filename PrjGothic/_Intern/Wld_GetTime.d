...
//		----------------------------------------------------
//
//		Wiedergabe der Zeit in Minuten und Ahnliches
...
//		Wld_GetHour () gibt die aktuelle Stunde zuruck
//		Wld_GetMinute () gibt die aktuelle Minute zuruck
//		Wld_GetTime() gibt die aktuelle Zeit im Format dhhmm zuruck
...
//		Wld_SubTime (VAR int days_sub, VAR int hours_sub, VAR int minutes_sub) stellt die Zeit um days_sub Tage, hours_sub Stunden und minutes_sub Minuten zuruck
//		Wld_GetTimePlus (VAR int days_add, VAR int hours_add, VAR int minutes_add) gibt die Uhrzeit in days_add Tagen, hours_add Stunden und minutes_add Minuten im Format dhhmm zuruck
//		Wld_GetTimeSub (VAR int days_sub, VAR int hours_sub, VAR int minutes_sub) gibt die Uhrzeit vor days_sub Tagen, hours_add Stunden und minutes_add Minuten im Format dhhmm zuruck
//		B_PrintTime (VAR int hhmm, VAR int x, VAR int y, VAR int t) Printet die Uhrzeit im Format hh:mm an Position (x|y) fur t Sekunden
//		Wld_IsExactTime (VAR int hhmm) liefert TRUE zuruck, wenn es genau hh:mm Uhr ist
//		SCRIPT_TIME () mindestens einmal die Sekunde vom Trigger TRIGGER_TIME aufrufen lassen, damit die Uhrzeit angezeigt wird
//		Wld_IsBeforeTime (VAR int dhhmm) liefert TRUE zuruck, wenn es vor d Tage, hh:mm Uhr ist
//		Wld_IsAfterTime (VAR int dhhmm) liefert TRUE zuruck, wenn es nach d Tage, hh:mm Uhr ist
//
//
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
FUNC void Wld_SubTime (VAR int days_sub, VAR int hours_sub, VAR int minutes_sub)		// Stellt die Zeit zuruck
{
	VAR int calc;
