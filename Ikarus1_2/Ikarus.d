...
func void MEM_SetCurrParserSymb (var int inst) {
    ...
    symOffset           = MEM_ReadIntArray (currSymbolTableAddress, inst); //èùåì àäðåñ inst â ñèìâîëüíîé òàáëèöå
    currParserSymOffset = MEM_ReadIntArray (contentSymbolTableAddress, currParserSymb);	//èùåì àäðåñ (instance currParserSymb) çíà÷åíèÿ â òàáëèöå

    MEM_WriteInt (currParserSymOffset + zCParSymbol_offset_offset, symOffset);	//çàïèñûâàåì â 
	// Èòîãî: èä¸ò ïåðåçàïèñü àäðåñà äàííûõ ê ïåðåìåíîé, êîíñòàíòå (÷òî èìååò ñèìâîëüíîå èìÿ)
};
...
func int MEMINT_GetBuf_8K_Sub() {
	/*NS - òîëüêî äëÿ êîìïèëÿöèè â GS3.15. â ôèíàëüíûé ñêðèïò ðàñêîììåíòèðîâàòü
    var int buf[2048];
	/*/
    var int buf[1024];
	//*/
...
//instance MEM_zCViewDialogChoice (zCViewDialogChoice);
...
func void MEM_InitGlobalInst() {
	...
	MEM_GamePtr = MEM_ReadInt (MEMINT_oGame_Pointer_Address);
	...
	//
	MEM_zCViewDialogMessage = _^(MEM_Game.array_view[1]);	//GAME_VIEW_CONVERSATION - ðàçãîâîð
	//MEM_zCViewDialogChoice = _^(MEM_Game.array_view[4]);	//GAME_VIEW_CHOICE - âûáîð
